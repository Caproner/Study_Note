...
+ 隐藏
  + 在多文件系统中所有未加static前缀的全局变量和函数都具有**全局可见性**
  + 也就是说，如果变量或函数没有加`static`的话，则这些变量或函数可以被其他头文件使用（使用`extern`关键字声明之后即可使用）
  + 而如果在变量或函数前面加上`static`的话，则可以对其他文件隐藏该成员
  + `static`作用于函数的话则只有隐藏这一效果
+ 将保存的内容持久化
  + `static`标记的变量在程序开始时会做唯一一次初始化，然后其变量的值便会跟全局变量一样存储在静态存储区中，即使该变量为局部变量，在退出当前变量作用范围时该变量的值也不会被重置
  + 如果该变量没有赋值的话默认初始化为0（与全局变量一致）

### extern "C"

`extern "C"`的作用是将其之后的东西按照C语言编译器的标准来进行解析：
...
这么做的原因是：C++编译器会进行符号修饰，这会改变其变量名或文件名的真实名字

如果所有的文件都是C++文件的话，由于所有文件的符号修饰规则是一样的，所以没有什么影响，而如果其中有一些文件是以C语言文件的形式进行编译的话，那么当一个C++文件要直接使用该C语言文件的变量或者include它的时候就会出现找不到变量或文件的错误，故需要加入`extern "C"`来修饰需要从C语言文件中引入的东西，使其不被C++的符号修饰规则所影响而能够正确找到目标