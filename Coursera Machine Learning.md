...
+ 添加外部特征：更多的特征可以使你的模型的参数更多，从而减低偏差
+ 添加多项式特征：原理同上
+ 降低正则化参数：正则化参数用于控制你的模型不会过拟合，故此时需要反过来降低正则化参数使正则项发挥更大的作用
+ 扩大神经网络：增加单层神经元也好，增加层数也好，目的是为了让你的网络复杂起来（从而增多参数）

对于过拟合而言：

+ 增大训练数据量：从学习曲线可以看出，增大训练数据量可以达到更理想的效果
+ 使用更少的特征：跟欠拟合反过来操作
+ 增大正则化参数：跟欠拟合反过来操作
+ 缩小神经网络：跟欠拟合反过来操作

### Building a Spam Calssifier

#### 解决机器学习问题的一般方法

首先，先选择一个简单快速但准确率不高的算法作为模型，在其基础上利用现有的数据和特征进行训练，并计算误差，绘制学习曲线等。通过这些信息会反映出当前数据存在的问题。

由于绝大多数模型在最开始的时候面对的问题是差不多的（例如特征提取不对，之类的），所以先使用一个跑起来快的算法来修正数据和特征是可行的。在修正之后，再使用复杂且准确率高的模型便可以节省时间。

#### 误差分析

在前面所说的修正过程中，除了针对过拟合和欠拟合的修正之外，还可以对验证结果中错误的或者偏差太大的数据手动进行误差分析，找出其共同点，据此再建立针对性的特征。

### Handing Skewed Data

#### 偏斜类

假设在某一个二分类问题上，你的模型有99%的准确率。看起来的确很棒。

但是事实上这个问题下分类为1的只有0.5%，那么这个时候设计一个只会输出0的算法的准确率就会达到99.5%。

但是显然，上面的算法并不是一个好的算法。

对于这类问题的数据就称为偏斜类。此时单从准确率上看的话并没有什么效果。

#### 查准率 and 召回率

现在假设四个变量：

+ True Positive（TP）：真阳性，指预测值为1且实际值也为1的数据数
+ True Negative（TN）：真阴性，指预测值为0且实际值也为0的数据数
+ False Positive（FP）：假阳性，指预测值为1但是实际值是0的数据数
+ False Negative（FN）：假阴性，指预测值为0但是实际值是1的数据数

于是就有：

+ 查准率=TP/(TP+FP)，指的是在预测为1的数据中有多少是准确的
+ 召回率=TP/(TP+FN)，指的是在实际为1的数据中有多少能够预测正确

使用这两个指标综合判断的话就可以避免上述问题。

例如上面的例子中，一个只会输出0的算法的召回率便是0。

#### 调整查重率和召回率

以逻辑回归为例。在逻辑回归中的函数给出的值是一个在[0,1]之间的值。

一般来说，我们认为高于0.5的就置为1，否则置为0。在这里0.5称为阀值。

但是事实上这个阀值是可调整的。而且有如下规律：

+ 阀值越高，表明FP会越小（FN会越大），查准率会变高（召回率会变低）
+ 阀值越低，表明FN会越小（FP会越大），召回率会变高（查准率会变低）

#### F值

查准率和召回率为两个值，并不能够直接用于判断哪个算法更好（例如说算法A高查准，算法B高召回，那选哪个？）

于是就有了F值。这里假设查准率为P，召回率为R，故有：
$$
F=2{\frac {PR} {P+R}}
$$
使用F值便可以解决这种选择困难（？）