...
一般来说，数据更经常会划分为训练集，验证集和测试集。（按6:2:2的比例划分）

假设现在手头有多个模型，需要从中选择一个模型。

一般的方法是，各自使用训练集训练，然后用验证集验证其误差，选取误差最小的模型。

不过此时并不能用验证集的误差来做最终误差使用，而是要另外再用一个测试集来计算误差，作为最终误差。

原因是，在使用验证集计算误差并选择模型的时候，这一步事实上就已经是另一种针对验证集进行的训练了。

于是最终得到的误差就跟训练集上出来的误差一样，不能够直接使用。

### Bias vs. Variance

#### 欠拟合 and 过拟合

欠拟合（underfitting），又称高偏差（High Bias），表现为训练误差和测试误差都很大。

过拟合（overfitting），又称高方差（High Variance），表现为训练误差小，但测试误差大。

#### 正则化

于是显然的，正则化参数λ过大的话便更容易发生欠拟合，过小的话则更容易发生过拟合。

#### 学习曲线

我们可以通过学习曲线来简单判断模型是过拟合还是欠拟合：

![9](img\Coursera Machine Learning\9.png)

上图为欠拟合点的情况。

![10](img\Coursera Machine Learning\10.png)

上图为过拟合的情况。可以看出如果再增加训练数据的可以达到更理想的效果。
